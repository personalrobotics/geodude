...
if TYPE_CHECKING:
    from geodude.parallel import GraspStateSnapshot
...
class SnapshotCollisionChecker:
    """Thread-safe collision checker using grasp snapshot.

    Like GraspAwareCollisionChecker, but uses an immutable GraspStateSnapshot
    instead of a live GraspManager. This allows multiple instances to run in
    parallel without interfering with each other.

    Each instance uses its own MjData, making it safe for concurrent use.
    """
    ...
    def __init__(
        ...
        """Initialize the snapshot-based collision checker.

        Args:
            model: MuJoCo model (shared, read-only)
            data: MjData for this checker (must be a private copy)
            joint_names: Names of joints to control
            grasp_snapshot: Immutable snapshot of grasp state
        """
        self.model = model
        self.data = data  # This is our own private copy
    ...
    def is_valid(self, q: np.ndarray) -> bool:
        ...
        Uses snapshot for grasp state and its own MjData for thread safety.
        ...
        # Update attached object poses based on snapshot
        self._update_attached_poses_from_snapshot()
        mujoco.mj_forward(self.model, self.data)
    ...
    def _update_attached_poses_from_snapshot(self) -> None:
        """Update poses of attached objects using snapshot data."""
        for obj_name, gripper_body_name, transform_tuple in self.grasp_snapshot.attachments:
            ...
            T_gripper_object = np.array(transform_tuple)
    ...
    def _count_invalid_contacts(self) -> int:
        ...
        Uses snapshot for grasp state queries.
        """
        invalid_count = 0
        ...
        for i in range(self.data.ncon):
            ...
            body2_is_arm = body2 in self._arm_body_ids
            body1_is_grasped = (
                body1_name is not None and self.grasp_snapshot.is_grasped(body1_name)
            )
            body2_is_grasped = (
                body2_name is not None and self.grasp_snapshot.is_grasped(body2_name)
            )
            ...
            if body1_is_robot and body2_is_robot:
                ...
                if self._is_gripper_object_contact(
                    ...
                    continue
                # Unexpected: arm self-collision or grasped object hitting arm
                invalid_count += 1
                continue

            # One is robot, one is environment
            invalid_count += 1
    ...
    def _is_gripper_object_contact(
        ...
        # Get the gripper body using snapshot
        gripper_body_name = self.grasp_snapshot.get_attachment_body(grasped_name)
        if gripper_body_name is None:
            return False

        # Find the gripper base body
        gripper_base_name = self._get_gripper_base_name(gripper_body_name)
        if gripper_base_name is None:
        ...
        # Check if the arm body in contact is part of the gripper
        gripper_body_ids = self._get_body_and_descendants(gripper_base_id)
        return arm_body_id in gripper_body_ids
